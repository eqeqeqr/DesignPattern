 ...
 * 换句话说:
 * 数据结构类它不创建任何对数据新的操作，只创建一个对外提供自己的数据的方法比如accept(访问者)，
 * 把对数据的操作让给访问者来操作，谁要用我的数据，就把访问者对象给数据结构类，数据结构类会把自己的实例（也就是对象）放到访问者提供的方法中比如 visitor.visitTeacher(this);
 * 然后数据结构类会调用访问者对象提供的访问方法，然后访问者类中会调用这个方法提供的参数（这里指数据结构类的实例）然后执行数据结构对象的相应方法（这里常见是指get set），然后对拿到的数据做新的操作
 * */
